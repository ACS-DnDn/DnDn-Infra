...
- `terraform/envs/prod`
  - 현재 유일한 실제 환경 엔트리
- `terraform/envs/dev`
  - 전역 리소스는 비활성화한 dev reserved 환경 엔트리
...
- `gitops/environments/dev`
  - dev root/child app mirror와 placeholder 값을 포함한 reserved GitOps 디렉터리
...
- `dev` scaffold를 실제 인프라와 Argo CD 환경으로 활성화할 시점
  - 특히 `route53`, `acm`, `eventbridge`, `s3_public` 같은 전역 리소스 활성화 전략과 placeholder 값 확정 필요
