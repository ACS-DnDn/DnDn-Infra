...
- `gitops/environments/prod/root/kustomization.yaml`
...
- `dndn-prod-root`가 직접 읽는 source는 `gitops/environments/prod/root/*` 입니다
- `gitops/apps/*.yaml`는 현재 `prod` 경로를 가리키도록 정리되어 있습니다
- `prod` root app은 최초 1회 수동 apply 후 Argo CD에서 관리하는 방식입니다
- `gitops/environments/prod/root/`는 Argo CD의 kustomize 경로 제한을 피하기 위해 self-contained source로 유지하는 편이 안전합니다
- `dndn-web`, `dndn-hr` ingress는 각 child app 쪽에서 namespace와 함께 관리하고, root app은 공용 진입점과 child app 선언 중심으로 유지합니다
- `dndn-external-secrets` app과 `ClusterSecretStore`는 `prod/root` source에서 함께 관리합니다
- `dndn-api`, `dndn-report` secret은 현재 AWS Secrets Manager + External Secrets Operator 기준으로 동기화되며, plain Secret manifest는 제거되었습니다
- Argo CD `Application`은 현재 `repoURL`을 직접 참조하며, repo credential manifest는 아직 이 레포에 선언적으로 포함되어 있지 않습니다
...
- 앱별 배포 단위 이름
- CI와 CD의 책임 분리
- `prod`의 `dndn-api`, `dndn-report` secret 주입 방식 (AWS Secrets Manager + ESO)
- 현재 prod 운영 절차 기준 (`docs/operations-runbook.md`)
...
- monitoring 설치 경로와 GitOps 포함 범위
- Argo CD repo credential 선언화 방식
- 최종 도메인과 인증서 연결값

## 7. Immediate Next Step

현재 문서 기준 다음 운영 정리 단계는 아래 순서가 가장 자연스럽습니다.

1. `prod` 앱 runtime 검증과 sync/health 기준 유지
2. monitoring 설치 경로 / values / ownership 정리
3. Argo CD repo credential 운영 정책 정리
4. `dev`, `staging` 환경 전략 확정
