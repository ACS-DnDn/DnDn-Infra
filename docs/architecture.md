...
- 현재 상태
  ...
  - Argo CD root app + child app + External Secrets 기반 `prod` 운영
  - `monitoring` namespace에 kube-prometheus-stack 계열 런타임 존재
- 목표 상태
  - Terraform 기반 공통 인프라
  - EKS 기반 앱 런타임
  - Argo CD 기반 GitOps 배포
  - `DnDn-App` 및 `DnDn-HR` 워크로드의 쿠버네티스 운영
  - GitHub Actions는 이미지 빌드와 푸시 중심으로 단순화
...
- `dndn-api`, `dndn-report` secret은 AWS Secrets Manager + External Secrets Operator로 주입 중
...
`DnDn-App/apps/report`는 하나의 이미지 태그를 산출하고, Kubernetes에서는 `report-api`, `report-worker` 두 런타임으로 분리해 배포하는 구조를 권장합니다.
...
- 앱별 Application 관리
- 환경별 overlay 분리
- 운영자가 kubectl 또는 helm 수동 배포 없이 Git 변경으로 배포
- dev / staging / prod 배포 흐름 표준화
- `app-of-apps` 구조로 환경 단위 운영
...
- docs
- `prod` root app을 포함한 GitOps 구조
- `dndn-external-secrets` app과 `ClusterSecretStore`
- `monitoring` namespace의 kube-prometheus-stack 계열 런타임
...
3. GitOps
   ...
   - Argo CD `app-of-apps`
   - `prod` root app bootstrap
4. DnDn-App
   - 컨테이너 이미지 산출
   - `web`, `api`, `worker`, `report` 이미지 산출
5. DnDn-HR
   - 관리자 포털 프론트엔드 이미지 산출
   - 메인 서비스 백엔드 연동
...
현재 기준으로는 아래 흐름을 권장합니다.

- `DnDn-App`, `DnDn-HR` GitHub Actions는 image build / push까지만 담당
- `DnDn-Infra`는 Helm/Kustomize, values, Argo CD `Application`을 관리
- 실제 EKS 반영은 Argo CD가 수행
