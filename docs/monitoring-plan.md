...
이 문서는 DnDn의 현재 모니터링 운영 상태와 후속 정리 방향을 짧게 정리합니다.
...
- Argo CD `Application`으로 관리되는 흔적은 없음
- chart/version/values source가 이 레포에 남아 있지 않음
- ownership이 모니터링 담당자 쪽에 더 가깝고, Infra 레포 기준 문서화가 덜 되어 있음
...
## 3. Future Direction Options

| Component | Role | Consider When |
| --- | --- | --- |
...
## 4. Why We Are Not Rebuilding Yet
...
## 5. Minimum Observability For Now
