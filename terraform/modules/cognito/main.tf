...
resource "aws_cognito_user_pool" "main" {
  ...
  admin_create_user_config {
    ...
    invite_message_template {
      ...
      email_message = "안녕하세요 {username}님, DnDn 플랫폼 계정이 생성되었습니다.<br><br>임시 비밀번호: {####}<br><br><a href='https://www.dndn.cloud/login'>로그인 하러 가기</a><br><br>첫 로그인 시 비밀번호를 변경해 주세요."
      sms_message   = "DnDn {username} 임시 비밀번호: {####}"
    }
  }
...
resource "aws_cognito_user_group" "leader" {
  name         = "leader"
  user_pool_id = aws_cognito_user_pool.main.id
