...
resource "aws_iam_role" "api" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${local.oidc_url}:sub" = "system:serviceaccount:${var.namespace}:dndn-api-sa"
...
resource "aws_iam_role" "worker" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${local.oidc_url}:sub" = "system:serviceaccount:${var.namespace}:dndn-worker-sa"
          "${local.oidc_url}:aud" = "sts.amazonaws.com"
        }
      }
    }]
...
resource "aws_iam_role" "reporter" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${local.oidc_url}:sub" = "system:serviceaccount:${var.namespace}:dndn-reporter-sa"
          "${local.oidc_url}:aud" = "sts.amazonaws.com"
        }
      }
    }]
